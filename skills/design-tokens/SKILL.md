...
2. **Read the brief.** Look for a design brief at `.agents/.design/*/DESIGN_BRIEF.md`. If multiple subfolders exist, use the most recently modified one, or ask the user which feature they are working on. If a philosophy is named, derive token values from its stated typography, color, layout, spacing, motion, and detail traits. If no brief exists, ask the user what direction they want.
