...
1. Look for an existing design brief at `.agents/.design/*/DESIGN_BRIEF.md`. If multiple subfolders exist, use the most recently modified one, or ask the user which feature they are working on. If no brief exists, ask the user what they are building and for whom.
...
4. Once you have a shared understanding, produce the design architecture document using the template below and save it as `DESIGN_ARCHITECTURE.md` in the same `.agents/.design/<feature-slug>/` subfolder as the design brief.
