...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/home/hschne/.config/tldraw/server.json`.
...
```bash
PORT=$(jq -r .port '/home/hschne/.config/tldraw/server.json'); TOKEN=$(jq -r .token '/home/hschne/.config/tldraw/server.json')
