...
1. Read the design brief. Look for `.agents/.design/*/DESIGN_BRIEF.md`. If multiple subfolders exist, use the most recently modified one, or ask the user which feature they are working on. Also check for `DESIGN_ARCHITECTURE.md` and a tokens file in the same subfolder. If none exist, ask the user to describe what they are building.
...
5. Save the task list as `TASKS.md` in the same `.agents/.design/<feature-slug>/` subfolder as the design brief.
...
Generated from: .agents/.design/<feature-slug>/DESIGN_BRIEF.md
Date: [date]
