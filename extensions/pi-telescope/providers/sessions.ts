...
function searchSessions(query: string): SessionInfo[] {
  ...
  return findSessionsFromFiles().filter((session) => {
    const text =
      `${session.name ?? ""} ${session.firstMessage} ${session.cwd}`.toLowerCase();
...
function buildRichPreview(
  ...
  // ── Header ──
  const name =
    session.name || session.firstMessage.split("\n")[0]?.trim() || "(unnamed)";
  ...
  lines.push(
    truncateToWidth(" " + th.fg("dim", `${msgs} · ${time} · ${cwdShort}`), w),
  );
