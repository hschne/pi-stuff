...
function buildEntries(
  ...
  const builtinCommandNames = new Set([
    ...
    "permissions",
    "leader-navigate",
    "leader-switch", // internal bridge commands
  ]);

  const customCommands = extCommands.filter(
    (c) => !builtinCommandNames.has(c.name),
  );
...
class LeaderKeyOverlay {
  ...
  render(width: number): string[] {
    ...
    if (this.view.type === "root") {
      lines.push(f.row(th.fg("accent", th.bold("Leader"))));
    } else {
...
export default function leaderExtension(pi: ExtensionAPI) {
