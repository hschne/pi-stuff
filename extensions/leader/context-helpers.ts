...
function isCommandCtx(ctx: ExtensionContext): ctx is ExtensionCommandContext {
  return (
    "switchSession" in ctx && typeof (ctx as any).switchSession === "function"
  );
}
 ...
 * falls back to the internal /leader-navigate command.
 */
export async function tryNavigateTree(ctx: ExtensionContext, targetId: string) {
  if (isCommandCtx(ctx)) {
    await ctx.navigateTree(targetId);
  } else {
    emitCommand(ctx, `/leader-navigate ${targetId}`);
  }
}

/**
 * Switch to a different session file. Uses the direct API when available,
 * falls back to the internal /leader-switch command.
 */
export async function trySwitchSession(
  ...
  if (isCommandCtx(ctx)) {
    ...
    emitCommand(ctx, `/leader-switch ${sessionPath}`);
  }
}
...
export function registerBridgeCommands(pi: ExtensionAPI) {
  pi.registerCommand("leader-navigate", {
  ...
  pi.registerCommand("leader-switch", {
        ...
        if (node.entry.type === "message") {
          ...
          if (msg.role === "user" || msg.role === "assistant") {
            if (typeof msg.content === "string") {
              ...
              if (text?.type === "text") preview = text.text.slice(0, 60);
            }
          }
        }
