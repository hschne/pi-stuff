...
export function registerDiffTools(pi: any): void {
  ...
  pi.registerTool({
    ...
    async execute(tid: string, params: any, sig: any, upd: any, ctx: any) {
      ...
      try {
        const { diffs, summary, totalAdded, totalRemoved } =
    ...
    renderCall(args: any, theme: any, ctx: any) {
      ...
      if (ctx.state._pk !== pk) {
        ...
        if (operations.length === 1) {
          ...
          const { diffs, totalAdded, totalRemoved } =
            summarizeEditOperations(operations);
          const summary = themedSummary(totalAdded, totalRemoved, theme);
          ...
          Promise.all(
            diffs.slice(0, maxShown).map((diff, index) =>
              renderSplit(diff, lg, previewLines, dc)
                .then(
                  (rendered) =>
                    `Edit ${index + 1}/${operations.length}\n${rendered}`,
                )
                .catch(
                  () =>
            ...
            .then((sections) => {
              ...
              const suffix =
                remainder > 0
                  ...
                  : "";
              ctx.state._pt = `${hdr}\n  ${theme.fg("muted", `${operations.length} edits`)} ${summary}\n\n${sections.join("\n\n")}${suffix}`;
              ctx.invalidate();
            })
            .catch(() => {
              if (ctx.state._pk !== pk) return;
    ...
    renderResult(result: any, opts: any, theme: any, ctx: any) {
      ...
      if (result.details?._type === "editInfo") {
        const { totalAdded, totalRemoved, editLine } = result.details;
        const loc =
      ...
      if (result.details?._type === "multiEditInfo") {
        const { totalAdded, totalRemoved, editCount, diffLineCount } =
          result.details;
        const dlInfo =
