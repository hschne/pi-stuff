...
> => {
  ...
  >((tui, theme, _kb, done) => {
    ...
    selectList.onSelect = (item) =>
        ...
        item.value as
          "reveal" | "quicklook" | "open" | "edit" | "addToPrompt" | "diff",
      );
...
const openInYazi = async (
  ctx: ExtensionContext,
  target: FileReference,
): Promise<void> => {
  const errorMessage = await ctx.ui.custom<string | null>(
    (tui, theme, _kb, done) => {
      const status = new Text(
        theme.fg("dim", `Opening ${target.display} in Yazi...`),
      );
      ...
      queueMicrotask(() => {
        ...
        try {
          const result = spawnSync("yazi", [target.path], {
            cwd: ctx.cwd,
            stdio: "inherit",
          });
          ...
          if (result.error) {
            ...
            message = `Yazi exited with status ${result.status ?? "unknown"}`;
          }
        } catch (error) {
          message =
            error instanceof Error ? error.message : "Failed to open Yazi";
        } finally {
          tui.start();
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerShortcut("alt+f", {
    description: "Open the latest file reference in Yazi",
    handler: async (ctx) => {
      const entries = ctx.sessionManager.getBranch();
